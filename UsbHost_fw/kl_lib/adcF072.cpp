...
void Adc_t::DisableCalibrateEnableSetDMA() {
    ...
    // Disable continuous mode and trigger
    ADC1->CFGR1 &= ~(ADC_CFGR1_CONT | ADC_CFGR1_EXTEN);
    // DMA
    dmaStreamSetMemory0(PDma, PBufW->data());
    dmaStreamSetTransactionSize(PDma, PBufW->size());
    dmaStreamSetMode(PDma, ADC_DMA_MODE);
...
AdcBuf_t& Adc_t::DoSingleMeasurement() {
    DisableCalibrateEnableSetDMA();
    ADC1->CFGR1 |= ADC_CFGR1_CONT; // Measure several times until needed oversampling count is done
    Periodic = false;
    AdcBuf_t* CurrBuf = PBufR;
    StartConversion();
    // Wait completion: when done, buffers are switched
    for(uint32_t t=0; t<270; t++) {
        chThdSleepMilliseconds(4);
        if(CurrBuf != PBufR) break;
    }
    Stop();
    return *PBufR;
}
